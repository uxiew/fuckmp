...
.text {
  ...
  line-height: 1.5;
      ...
      expect(wxmlContent).toMatch(/data-v-[a-z0-9]{6}/) // 作用域属性

      // 验证生成的 WXSS 包含作用域选择器
      const wxssContent = await readTestFile('output/components/ScopedTest.wxss')
      expect(wxssContent).toMatch(/\.scoped-component\[data-v-[a-z0-9]{6}\]/)
      expect(wxssContent).toMatch(/\.title\[data-v-[a-z0-9]{6}\]/)
      expect(wxssContent).toMatch(/\.content\[data-v-[a-z0-9]{6}\]/)
      expect(wxssContent).toMatch(/\.text\[data-v-[a-z0-9]{6}\]/)
    })
  })
})
